...
[
  {
    ...
    "evidence": "client.chat.completions.create(model=model_name, messages=messages, max_tokens=max_tokens, temperature=temperature, timeout=timeout, stream=true)"
  }
]
```
...
### 2026-05-26
- **即兴写作模式** — 项目可在「大纲模式」（先写全书蓝图再逐章生成）和「即兴模式」（每章口述意图 → 单章蓝图 → 可选细纲 → 正文）之间切换。即兴模式不依赖全书蓝图，靠**伏笔池（open_threads）**维持连续性，finalize 后自动更新已埋未收 / 已收未结 / 待开发三段结构。
- **单章蓝图/细纲修订** — 即兴模式下，单章蓝图和细纲都支持基于建议修订（"仅调整指出的字段，其他保留"）。
- **创作工坊模式横幅** — 顶部彩色横幅显示当前工作流模式，一键切换。
- **项目备份与还原** — 项目顶栏新增「备份管理」面板：一键创建带备注的快照、列表查看（时间/备注/大小）、一键还原（还原前自动备份当前状态）、删除旧备份。
- **文件管理可编辑** — 文件管理页面打开 .txt/.json/.md 后可进入编辑模式，"未保存"指示器与保存/取消按钮齐全。
...
| 功能模块 | 说明 |
|---------|------|
| 双工作流模式 | 大纲模式（全书规划 → 逐章生成）/ 即兴模式（按章口述意图 → 单章蓝图 → 正文），可随项目切换 |
| 小说架构生成 | 核心种子 / 世界观构建 / 角色动力学 / 情节架构 |
| 章节目录规划 | 悬念节奏曲线 / 伏笔管理 / 强度分布设计 |
| 智能章节生成 | 知识库检索 / 前文摘要 / 上下文连贯性保障 |
| 即兴写作 | 不依赖全书蓝图，靠伏笔池（open_threads）维持连续性，适合心中无完整规划的创作 |
...
| 项目管理 | 多项目并行管理，独立配置与存储，支持备份/还原快照 |
